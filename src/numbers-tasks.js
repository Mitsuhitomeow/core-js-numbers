...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorLength1 = x1 ** 2 + y1 ** 2;
  const vectorLength2 = x2 ** 2 + y2 ** 2;
  return Math.acos(scalar / (vectorLength1 * vectorLength2));
}
...
function getSumOfDigits(num) {
  return num.length === 1 ? num : [...`${num}`].reduce((acc, el) => +acc + +el);
}
...
function getCountOfOddNumbers(number) {
  const positiveNum = number > 0 ? number : number * -1;
  let count = 0;
  for (let i = 1; i <= positiveNum; i += 2) {
    count += 1;
  }
  return count;
}
